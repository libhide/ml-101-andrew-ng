...
% Final unregularised cost
J = 1/m * J;

% Adding regularised term
squared_Theta1 = Theta1 .^ 2;
squared_Theta2 = Theta2 .^ 2;
reg_term = (lambda / (2 * m)) * (sum(sum((squared_Theta1(:, 2:end)))) + sum(sum(squared_Theta2(:, 2:end))));
J = J + reg_term;
...
for t = 1:m
    ...
    delta_2 = delta_2(2:end);  % removing delta for bias unit
    
    delta_accum_2 = delta_accum_2 + (delta_3' * a_2);
    delta_accum_1 = delta_accum_1 + (delta_2' * a_1);
end
...
Theta1_grad(:, 2:end) = Theta1_grad(:, 2:end) + Theta1_reg_term;
Theta2_grad(:, 2:end) = Theta2_grad(:, 2:end) + Theta2_reg_term;
